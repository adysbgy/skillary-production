...
const legacy = manifest.protectedDependencies.filter((item) => item.decision !== "DELETED").map((item) => item.sourceFile);
for (const file of [...clean, ...shared, ...legacy]) await access(file);
for (const file of clean) {
  const source = await readFile(file, "utf8");
  if (source.includes("@/app/v2/")) throw new Error(`Clean route imports v2 route module: ${file}`);
  if (!source.includes("@/features/")) throw new Error(`Clean route lacks shared feature import: ${file}`);
}
for (const file of shared) {
  const source = await readFile(file, "utf8");
  if (source.includes("@/app/")) throw new Error(`Shared feature imports route module: ${file}`);
}
for (const file of legacy) {
  const source = await readFile(file, "utf8");
  if (!source.includes("@/features/")) throw new Error(`Legacy wrapper lacks shared feature import: ${file}`);
}
console.log(`Route-module dependency audit passed: clean routes own shared features; ${legacy.length} legacy wrappers remain.`);
