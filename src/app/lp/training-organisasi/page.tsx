...
const TESTIMONIALS = [
  {
    quote: "Sekarang setiap batch ada laporan kehadiran dan nilai otomatis — tim HR tidak perlu rekap manual lagi.",
    name: "Siti Rahmawati",
    role: "HR Manager",
    org: "Perusahaan Manufaktur",
  },
  {
    quote: "Sertifikat digital yang bisa diverifikasi bikin peserta lebih serius mengikuti pelatihan sampai selesai.",
    name: "Andri Pratama",
    role: "L&D Lead",
    org: "Institusi Keuangan",
  },
];
...
export default function TrainingOrganisasiLpPage() {
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <section id="form-konsultasi" className="px-5 md:px-8 py-12" style={{ background: "rgb(255, 248, 241)" }}>
        <div className="max-w-4xl mx-auto grid lg:grid-cols-5 gap-8 items-start">
          ...
          <div className="hidden lg:block lg:col-span-2 space-y-5">
            ...
            {TESTIMONIALS.map((t) => (
              <div key={t.name} className="bg-white rounded-2xl p-6" style={{ border: "1.5px solid rgb(240, 217, 200)" }}>
                <p className="text-sm text-[#334155] leading-relaxed mb-4">&ldquo;{t.quote}&rdquo;</p>
                <p className="text-sm font-bold text-[#0F172A]">{t.name}</p>
                <p className="text-xs text-[#64748B]">{t.role} &middot; {t.org}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── Section 6: Testimonials — mobile only ── */}
      <section className="lg:hidden px-5 md:px-8 py-10 bg-white">
        <div className="max-w-xl mx-auto space-y-4">
          {TESTIMONIALS.map((t) => (
            <div key={t.name} className="rounded-2xl p-5" style={{ background: "rgb(255, 251, 245)", border: "1.5px solid rgb(240, 217, 200)" }}>
              <p className="text-sm text-[#334155] leading-relaxed mb-3">&ldquo;{t.quote}&rdquo;</p>
              <p className="text-sm font-bold text-[#0F172A]">{t.name}</p>
              <p className="text-xs text-[#64748B]">{t.role} &middot; {t.org}</p>
            </div>
          ))}
